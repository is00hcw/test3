...
public interface MemberRepository
  ...
  @SqlTemplate(
    name = "findByName",
    sql = "select * from Member where  {{#memberName}}memberName=:memberName{{/memberName}}"
  )
  ...
  @SqlTemplate(
    name = "updateMember",
    sql = "update Member set memberName=:memberName where memberAge=:memberAge"
  )
  @Modifying
   ...
   */
  @SqlTemplate(
    name = "findMemberInDep",
    sql =
        "SELECT	memberName AS NAME, memberAge AS age , depType AS dType FROM Member m,Department d WHERE m.depName=d.depName and memberName=:memberName"
  )
   ...
   * @return
   */
  @SqlTemplate(name = "findMemberOrderBy", sql = "select * from Member order by memberAge desc")
  Page<MemberDTO> findMemberOrderBy(@Param("pageable") Pageable pageable);
}
