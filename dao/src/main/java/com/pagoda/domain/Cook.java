...
@DynamicInsert
@Table(name = "Cook")
@SQLDelete(sql = "update Cook set deleted = true where id = ? and version = ?")
...
public class Cook extends CookDTO implements Serializable {
  ...
  static {
    ...
    mapperFactory
        .classMap(Cook.class, CookDTO.class)
        .mapNulls(false)
  ...
  public static Cook convertDTO(CookDTO dto) {
    return mapper.map(dto, Cook.class);
  }

  public static Iterable<Cook> batchConvertDTO(Iterable<CookDTO> dtos) {
    List<Cook> entities = new ArrayList<>();
    dtos.forEach(d -> entities.add(convertDTO(d)));
    return entities;
  }

  public static Iterable<CookDTO> batchConvert(Iterable<Cook> entities) {
    List<CookDTO> dtos = new ArrayList<>();
    entities.forEach(e -> dtos.add(e));
    return dtos;
  }

  public static Converter<Cook, CookDTO> DTO_CONVERTER =
      new Converter<Cook, CookDTO>() {

        @Override
        public CookDTO convert(Cook source) {
          return source;
        }
      };
}
